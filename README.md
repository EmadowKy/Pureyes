# Pureyes 视频问答系统

一个基于 AI 的视频问答平台，你可以上传视频，然后针对视频内容提问，AI 会分析视频并给出答案。

## 项目结构

```
Pureyes/
├── backend/     # 后端服务（Flask）
└── frontend/    # 前端页面（Vue3 + Vite）
```

## 环境要求

- **FFmpeg**：视频处理依赖，系统需要安装 FFmpeg 并配置到 PATH 环境变量
- Python 3.8+（后端）
- Node.js 16+（前端，自带 npm）

## 快速启动

### 1. 启动后端

```bash
cd backend
pip install -r requirements.txt
python run.py
```

后端启动后会显示 `Running on http://127.0.0.1:5000`，表示服务正常运行。

### 2. 启动前端

```bash
cd frontend
npm install
npm run dev
```

前端启动后访问 `http://127.0.0.1:5173`

## 功能说明

### 用户功能

- **注册登录**：创建账号、登录系统
- **个人中心**：查看用户信息、视频使用量和存储空间限制
- **退出登录**：安全退出当前账号

### 视频管理

- **上传视频**：支持 MP4 格式，单个文件最大 500MB
- **视频列表**：查看已上传的所有视频
- **视频播放**：点击视频直接播放
- **重命名**：给视频起个方便识别的名字
- **删除视频**：不需要的视频可以删掉
- **使用限制**：每个账号最多 30 个视频，总共 2GB 存储空间

### 视频问答

- **提问**：选中视频后输入问题，AI 会分析视频内容并回答
- **时间戳跳转**：回答中会标注具体时间点，点击可跳转到视频的对应位置
- **实时进度**：提问后能看到 AI 分析的进度
- **历史记录**：所有问答都会保存，可以随时翻看
- **导出记录**：支持把问答记录导出为 JSON 文件
- **统计数据**：能看到总提问数、成功数、失败数

## 技术栈

- 前端：Vue3、Vite、Element Plus
- 后端：Flask
- AI：集成大语言模型进行视频内容分析

## MVA 多视频理解模型工作原理

MVA（Multi-Video Agent）是一个多视频联合理解模型，核心是用视觉语言模型（Qwen-VL）来分析和理解多个视频的内容。整个过程分为三个阶段。

### 模型基础

模型使用 Qwen-VL 作为视觉语言 backbone，能够同时处理图像和文本输入。分析视频时会把视频切分成帧（frame），然后逐帧理解。

每个视频维护两个核心数据：

- **Frame Bank**：存放该视频中挑出来的关键帧，每帧带一个分数（0-1），代表这帧和问题的相关程度
- **Description**：文字描述，记录模型对视频各时间段内容的理解

***

### 第一阶段：问题分析

在正式处理视频之前，模型会先分析用户的问题，理解问题在问什么、需要关注什么类型的视觉信息。这一步输出的"问题分析"会贯穿后续所有环节，作为 ToolAgent 和 DescAgent 的参考。

***

### 第二阶段：初始化

对每个视频依次做初始化。

**1. 均匀采样帧**

从视频开头到结尾，均匀地抽出8帧

**2. 生成初始描述和评分**

DescAgent 接收这 8 帧图像，结合问题、其他视频的描述（如果有的话），生成一段文字描述这段视频的内容，同时给出一个 0-1 之间的分数。分数越高代表这段视频和问题的关联度越高。

初始描述格式类似：`0.00-120.00: 这段视频展示了... (Score: 0.65)`

**3. 建立 Frame Bank**

每帧的初始分数设为 0.5，和描述一起存入 Frame Bank。Frame Bank 有容量限制（默认 16 帧），超出时按分数从高到低保留。

初始化完成后，每个视频都有一个全局的初始描述，以及一批带有分数的关键帧。

***

### 第三阶段：主循环迭代

这是模型的核心推理过程，最多循环 **20** 次。

**1. 计算优先级，选择视频**

每个视频有一个动态计算的"优先级分数"，由当前分数和加速度（进步速度）共同决定：

$$P = e^{k \cdot a} \cdot (1 - s)^t$$

其中 $a$ 是加速度（分数变化率），$s$ 是当前分数，$k$ 和 $t$ 是权重参数。分数低（还有提升空间）且进步快的视频会被优先选中处理。

**帧分数评分（ToolAgent）**

每帧的分数由 ToolAgent 结合问题的上下文，从帧图像中判断该帧与问题的相关程度。输出格式为：

```
Scores: [0.65 0.32 0.78 0.45 ...]
```

每个分数都是 0-1 之间的小数，分数越高代表该帧与问题的关联度越大。如果模型未能给出足够的分数，则默认填充 0.5。

**视频片段评分（DescAgent）**

DescAgent 对当前采样的视频片段给出综合评分，输出格式为：

```
Score: 0.72
Video Terminated: False
Global Terminated: False
```

评分标准：

- **0.00**：该片段完全没有有用信息，需要继续探索
- **0.50**：该片段提供了一部分有用线索
- **1.00**：该片段已经包含回答问题所需的充分证据

即使分数为 1.0，也可能在其他视频还需要探索的情况下继续处理（多视频联合分析）。

**加速度计算**

加速度用于衡量模型的"进步速度"，即每次迭代后分数的变化率：

$$a = \frac{s\_{new} - s\_{old}}{\max(s\_{old}, 0.01)}$$

其中 $s\_{new}$ 是当前片段的新分数，$s\_{old}$ 是上一次迭代的分数。如果旧分数过低（小于 0.01），分母会固定为 0.01 防止除零。加速度越高，说明最新采样的片段质量提升越明显。

**2. ToolAgent 决策：采样位置和采样策略**

选中的视频交给 ToolAgent 分析。Agent 拿到：

- Frame Bank 中的已有帧（带分数和时间戳）
- 当前批次刚采样的帧（需要评分）
- 该视频的现有描述、其他视频的描述

Agent 输出一组帧的分数，以及一个决策（Option 1-6）。实际代码中，Option 1-4 均按指定时间区间采样，行为一致；Option 5 为全局采样；Option 6 终止分析：

- **Option 1**：当前区间可能有用，但也想看看其他部分 → 采样目标区间内帧
- **Option 2**：当前区间有用且需要进一步探索 → 采样目标区间内帧
- **Option 3**：保留选项（提示模型不选用，但代码中实际行为同 Option 1/2）
- **Option 4**：保留选项（提示模型不选用，但代码中实际行为同 Option 1/2）
- **Option 5**：全局均匀采样（探索新区域）。每次采样后偏移量会递增，确保每次全局采样落在视频不同位置，避免重复探索同一区域
- **Option 6**：终止对该视频的分析

同时输出目标时间区间 `[target_start, target_end]`。

**3. 按决策采样新帧**

根据 Option 和目标区间，从视频中抽出 8 帧。

**4. DescAgent 描述新帧并判断终止**

新帧交给 DescAgent，生成新的文字描述，追加到视频的完整描述后面。同时判断：

- **Video Terminated**：该视频已经充分分析，可以终止
- **Global Terminated**：所有视频整体已经充分，可以提前结束

**5. 更新状态**

- 更新 Frame Bank：新帧加入，按分数排序，超容量的淘汰低分帧
- 更新 Description：追加新的描述段落
- 更新分数和加速度
- 重新计算优先级

**6. 终止条件**

满足以下任一条件时循环结束：

- 所有视频都被标记终止
- 达到最大迭代次数（**20** 次）
- 某视频得分超过 0.8（认为已经充分理解）
- 某视频加速度持续过低（认为没什么可挖的了）

***

### 第四阶段：生成最终回答

所有视频分析完毕后，用最终保留的 Frame Bank 和所有 Description 生成答案。

**1. 筛选关键帧**

Frame Bank 中分数低于 0.5 的帧会被过滤掉。

**2. 给帧加水印**

为保留的帧添加视频标签和时间戳，方便最终回答中引用具体时间点。

**3. 调用 Qwen-VL 生成回答**

把筛选后的关键帧、累积的所有描述、原始问题一起发给视觉语言模型，生成最终的自然语言回答。回答中会包含 `[video:X, time:XX:XX]` 格式的时间戳标记，用户点击可以跳转到对应视频的对应时间点。

***

### 处理流程图

```
用户提问
    │
    ▼
问题分析 ─────────────────────────────────────────┐
    │                                               │
    ▼                                               │
初始化（每个视频）                                   │
  ├─ 均匀采样 8 帧                                  │
  ├─ DescAgent 生成描述 + 打分                       │
  └─ 建立 Frame Bank                                │
    │                                               │
    ▼                                               │
主循环迭代（最多 10 次）                             │
  ├─ 计算优先级 → 选中最优视频                        │
  ├─ ToolAgent 决策采样位置 + 策略                   │
  ├─ 采样新帧                                       │
  ├─ DescAgent 描述新帧 + 判断终止                    │
  └─ 更新 Frame Bank / Description / 分数            │
    │                                               │
    ▼                                               │
最终回答生成                                         │
  ├─ 筛选关键帧（分数 ≥ 0.5）                        │
  ├─ 添加水印（视频标签 + 时间戳）                    │
  └─ Qwen-VL 生成答案 ───────────────────────────────┘
    │
    ▼
返回带时间戳标记的答案
```

## 用户账号系统

Pureyes 使用基于 JWT 的用户认证系统，支持用户注册、登录、Token 刷新和安全退出。

### 数据模型

**User 模型**（SQLAlchemy）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | Integer | 用户唯一ID（主键） |
| username | String(64) | 用户名，唯一，索引 |
| email | String(128) | 邮箱，唯一，索引，可为空 |
| password_hash | String(255) | 密码哈希（Werkzeug） |
| role | String(20) | 角色：user/admin |
| is_active | Boolean | 账号是否启用 |
| total_video_size | BigInteger | 已使用存储空间（字节） |
| video_count | Integer | 已上传视频数量 |
| created_at/updated_at | DateTime | 创建/更新时间 |

**TokenBlacklist 模型**

| 字段 | 类型 | 说明 |
|------|------|------|
| id | Integer | 主键 |
| jti | String(64) | JWT ID，唯一，索引 |
| created_at | DateTime | 加入黑名单时间 |

### 认证流程

**注册 `/api/auth/register`**

1. 验证用户名（必填，≥3字符）、密码（必填，≥6字符）、邮箱（可选）
2. 检查用户名和邮箱是否已被占用
3. 密码使用 Werkzeug 的 `generate_password_hash` 加密存储
4. 创建用户，默认角色为 `user`，返回用户信息

**登录 `/api/auth/login`**

1. 验证用户名和密码非空
2. 查询用户，检查账号是否激活
3. 使用 `check_password_hash` 验证密码
4. 生成两种 JWT Token：
   - **Access Token**：包含用户身份、角色、用户名，有效期短
   - **Refresh Token**：仅包含用户身份，用于续期 Access Token
5. 返回 Token 和用户信息，存储到前端 localStorage

**Token 刷新 `/api/auth/refresh`**

1. 使用 Refresh Token 请求
2. 验证 Token 有效性，检查用户是否存在且激活
3. 生成新的 Access Token 返回

**登出 `/api/auth/logout`**

1. 将当前 Token 的 JTI 加入黑名单
2. 后续请求携带该 Token 会被拒绝

### 前端 Token 管理

- **存储位置**：localStorage（`access_token`、`refresh_token`、`user_info`）
- **自动刷新**：检测到 Access Token 过期时，自动用 Refresh Token 申请新 Token
- **Token 解析**：解码 JWT payload 检查 exp 字段判断是否过期
- **请求拦截**：API 请求自动在 Header 注入 `Authorization: Bearer <token>`

### 权限控制

**角色装饰器 `role_required(*roles)`**

- 配合 `@jwt_required()` 使用
- 从 JWT Claims 获取当前用户角色
- 角色不在允许列表中返回 403 权限不足

### API 端点

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | /api/auth/register | 注册 | 否 |
| POST | /api/auth/login | 登录 | 否 |
| POST | /api/auth/refresh | 刷新Token | Refresh Token |
| POST | /api/auth/logout | 登出 | Access Token |
| GET | /api/users/me | 获取当前用户信息 | Access Token |
| PUT | /api/users/me | 更新当前用户（邮箱/密码） | Access Token |
| DELETE | /api/users/me | 删除当前用户 | Access Token |
| GET | /api/users/config | 获取用户配置限制 | Access Token |

## 视频管理系统

Pureyes 的视频管理模块负责处理用户上传视频的存储、播放和管理。系统采用前后端分离架构，前端使用 Vue3 + Element Plus，后端使用 Flask + SQLAlchemy。

### 数据模型

**Video 模型**

| 字段 | 类型 | 说明 |
|------|------|------|
| video_id | String(32) | 主键，UUID 前 32 位 |
| uid | String(32) | 所属用户 ID |
| video_name | String | 视频显示名称 |
| video_path | String | 视频文件路径 |
| upload_time | DateTime | 上传时间 |
| duration | Float | 视频时长（秒） |
| file_size | BigInteger | 文件大小（字节） |
| is_ticked | Boolean | 是否被勾选 |

### 上传视频原理

**流程**

```
前端选择文件 → 验证格式/大小 → FormData 提交 → 后端多重校验 → 保存文件 → 入库 → 返回结果
```

1. **前端校验**
   - 文件格式限制为 MP4
   - 单个文件大小不超过 500MB
   - 使用 Element Plus 的 el-upload 组件

2. **后端校验（3 重验证）**
   - 验证 1：单个文件大小检查
   - 验证 2：用户视频数量限制（默认最多 30 个）
   - 验证 3：用户总存储空间限制（默认 2GB）

3. **文件处理**
   - 使用 UUID 生成唯一 video_id（取前 32 位）
   - 文件名格式：`{video_id}.mp4`
   - 存储路径：`backend/uploads/{video_id}.mp4`
   - 使用 OpenCV (`cv2.VideoCapture`) 提取视频时长

4. **数据库操作**
   - 创建 Video 记录
   - 更新 User 表的 `total_video_size` 和 `video_count` 字段

### 删除视频原理

**流程**

```
前端确认删除 → 发送 video_id → 后端验证归属 → 删除物理文件 → 更新用户统计 → 删除数据库记录
```

1. 验证视频属于当前用户（防止跨用户删除）
2. 删除物理视频文件 `uploads/{video_id}.mp4`
3. 更新用户的存储统计（减去该视频占用的空间和数量）
4. 删除数据库 Video 记录

### 重命名视频原理

**流程**

```
前端输入新名称 → 发送 video_id + new_name → 后端验证归属 → 更新数据库 → 返回新信息
```

- 仅修改数据库中的 `video_name` 字段
- 物理文件名保持不变（`{video_id}.mp4`）
- 需要验证视频文件实际存在

### 视频播放原理

**视频流服务 (`video_stream_routes.py`)**

```
前端请求 → 路径规范化 → 格式检查 → 兼容则直传 / 不兼容则转码 → 返回视频流
```

1. **路径规范化**
   - 支持多种路径格式输入
   - 统一转换为 `uploads/{video_id}.mp4` 相对路径
   - 防止路径遍历攻击

2. **格式兼容性检查**
   - 使用 ffprobe 检测视频编码信息
   - 支持 H.264 High/Baseline Profile + AAC 格式直接播放
   - 其他格式需要转码

3. **实时转码**
   - 使用 ffmpeg 转码为浏览器兼容格式
   - 输出配置：H.264 High Profile + AAC 音频 + moov 前置
   - 线程池异步执行，最多 2 个并发转码任务

4. **缓存机制**
   - 转码后的文件缓存到 `temp/transcoded/` 目录
   - 最多缓存 5 个转码文件
   - LRU 策略自动清理旧缓存

### 前端视频组件

**VideoPlayer 组件 (`VideoPlayer.vue`)**

- 基于原生 HTML5 `<video>` 元素封装
- 支持播放/暂停/错误重试
- 监听多种错误类型（网络错误、解码失败、格式不支持等）
- 提供 `retry()` 方法用于自动重新加载

**视频列表管理 (`IntegratedQA.vue`)**

- 左侧边栏展示视频列表，支持滚动选择
- 视频卡片显示：名称、时长、大小、上传时间
- 操作按钮：重命名、删除
- 点击视频切换播放器内容

### API 端点

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | /api/video-manage/upload | 上传视频 | Access Token |
| POST | /api/video-manage/delete | 删除视频 | Access Token |
| POST | /api/video-manage/rename | 重命名视频 | Access Token |
| POST | /api/video-manage/tick | 勾选视频 | Access Token |
| GET | /api/video-manage/list | 获取视频列表 | Access Token |
| GET | /api/video-manage/info | 获取单个视频信息 | Access Token |
| GET | /api/video/\<path\> | 视频流播放 | - |

### 配置管理

视频上传限制通过 `backend/configs/qa_storage.yaml` 配置文件管理：

```yaml
video:
  max_single_video_size: 524288000    # 单个视频最大 500MB
  max_storage_per_user: 2147483648    # 用户总存储最大 2GB
  max_videos_per_user: 30              # 用户最多 30 个视频
```

## 视频问答功能

视频问答模块提供了一套完整的交互流程，支持勾选视频提问、实时展示分析过程、管理问答记录等功能。

### 功能概览

| 功能 | 说明 |
|------|------|
| **勾选视频** | 从视频列表勾选要提问的视频，支持多选 |
| **提问** | 输入问题后提交，后台异步处理 |
| **实时进度** | 通过 SSE 流式推送分析进度，前端实时展示 |
| **历史记录** | 保存所有问答记录，支持分页查看 |
| **记录详情** | 查看单条记录的完整问答内容和分析过程 |
| **删除记录** | 删除不需要的问答记录 |
| **导出记录** | 将问答记录导出为 JSON 文件 |
| **统计摘要** | 查看总提问数、成功数、失败数统计 |

### 勾选视频提问流程

**前端交互**

1. 用户在视频列表左侧勾选一个或多个视频（通过视频卡片的复选框）
2. 被勾选的视频 `is_ticked` 状态设为 `true`，视频卡片显示勾选标识
3. 用户在问答区域输入问题
4. 点击提交按钮，调用 `POST /api/qa/ask` 接口

**后端处理**

1. 验证用户身份（JWT）和问题内容
2. 生成唯一 `task_id` 作为任务标识
3. 创建临时记录（状态为 `processing`），立即保存到存储
4. 启动后台线程异步调用 MVA 模型处理
5. 返回 `task_id` 和 `task_token` 给前端

**数据模型**

问答记录结构：

```json
{
  "record_id": "任务唯一ID",
  "timestamp": "创建时间 ISO 格式",
  "question": "用户问题",
  "video_paths": ["视频路径列表"],
  "success": null,
  "model_result": {
    "answer": "AI 正在分析中...",
    "status": "processing"
  },
  "status": "processing"
}
```

> **注意**：`success` 字段在处理中为 `null`，完成后为 `true`（成功）或 `false`（失败）。

### 实时进度展示

**SSE 流式推送**

后端 `/api/qa/task/<task_id>/stream` 接口使用 Server-Sent Events（SSE）推送进度：

```
data: {"type": "connected", "task_id": "xxx", "submit_time": 1234567890}
data: {"type": "progress", "data": {"stage": "initialization", ...}}
data: {"type": "progress", "data": {"stage": "iteration", "iteration": 1, ...}}
data: {"type": "complete", "status": "completed", "result": {...}}
```

**前端展示**

- 使用 `EventSource` 订阅进度流
- `RealtimeStreamProcessFlow` 组件实时渲染进度
- 完成后自动切换到 `ProcessFlow` 组件显示完整分析过程

### 问答记录管理

**存储结构**

每个用户的记录存储在 `output/qa_records/{uid}/qa_records.json`，按 uid 隔离存储。

**记录操作 API**

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/qa/records | 获取记录列表（分页） |
| GET | /api/qa/record/{id} | 获取单条记录详情 |
| DELETE | /api/qa/record/{id} | 删除指定记录 |
| GET | /api/qa/summary | 获取统计摘要 |
| GET | /api/qa/export | 导出所有记录为 JSON |
| GET | /api/qa/task/{id}/stream | SSE 流式推送进度（需要 task_token） |
| GET | /api/qa/task/{id}/status | 获取任务状态 |
| GET | /api/qa/task/{id}/progress | 获取任务实时进度 |

**删除记录**

前端调用 `DELETE /api/qa/record/{record_id}`，后端根据 uid 和 record_id 查找并删除对应记录。

**导出记录**

调用 `GET /api/qa/export?format=json`，后端读取用户所有记录，生成临时 JSON 文件并返回下载。

### 前端组件结构

**IntegratedQA.vue 主组件**

- 视频列表区域（左侧）：展示可勾选的视频卡片
- 问答区域（右侧）：输入问题、提交按钮、实时进度展示
- 记录列表视图：分页展示历史问答记录
- 记录详情视图：展示单条记录的完整内容

**关键函数**

```javascript
// 勾选视频
await videoApi.tickVideos({ video_ids: [id], is_ticked: true })

// 提交问题
const res = await qaApi.askQuestion({ question, video_paths })

// 订阅实时进度
qaApi.subscribeTaskProgress(taskId, onProgress, onComplete, onError, token)

// 查看记录详情
const res = await qaApi.getRecord(recordId)

// 删除记录
await qaApi.deleteRecord(recordId)

// 导出记录
await qaApi.exportRecords('json')

// 获取统计
const res = await qaApi.getSummary()
```

### 后端关键模块

**QAManager（qa_manager.py）**

管理问答记录的持久化存储：

- `save_temp_record()`：创建临时记录
- `update_record()`：更新记录状态和结果
- `delete_record()`：删除记录
- `get_user_records()`：获取用户所有记录
- `get_record_summary()`：获取统计数据

**QA 路由（routes.py）**

处理所有 QA 相关 HTTP 请求：

- 异步任务管理（内存 + 持久化）
- SSE 流式推送进度
- 记录 CRUD 操作
- 导出功能

### 配置管理

问答记录存储通过 `backend/configs/qa_storage.yaml` 配置：

```yaml
storage:
  storage_dir: "output/qa_records"    # 记录存储目录
  backup_dir: "output/qa_backups"     # 备份目录
  max_records_per_user: 100           # 每用户最大记录数
  enable_backup: true                  # 是否启用备份
```


